...
#include "LEGameModel/GameManager.h"
...
void WorldStateMachine::Start(String manifestPath, String gameStyleID, Task onManifestLoaded)
{
	m_onManifestLoaded = std::move(onManifestLoaded);
...
void WorldStateMachine::LoadingTick(Time dt)
{
	...
	if (m_bLoaded)
	{
		if (m_onManifestLoaded)
		{
			m_onManifestLoaded();
			m_onManifestLoaded = nullptr;
		}
